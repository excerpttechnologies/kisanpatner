...
exports.registerTransport = async (req, res) => {
  try {
    ...
    const existingTransporter = await Farmer.findOne({ 
      'personalInfo.mobileNo': mobileNo 
    });
    ...
    if (req.files) {
      if (req.files.rcBook) {
        documents.rcBook = `/uploads/${req.files.rcBook[0].filename}`;
      }
      if (req.files.insuranceDoc) {
        documents.insuranceDoc = `/uploads/${req.files.insuranceDoc[0].filename}`;
      }
      if (req.files.pollutionCert) {
        documents.pollutionCert = `/uploads/${req.files.pollutionCert[0].filename}`;
      }
      if (req.files.permitDoc) {
        documents.permitDoc = `/uploads/${req.files.permitDoc[0].filename}`;
      }
      if (req.files.driverLicense) {
        documents.driverLicense = `/uploads/${req.files.driverLicense[0].filename}`;
      }
      if (req.files.driverPhoto) {
        documents.driverPhoto = `/uploads/${req.files.driverPhoto[0].filename}`;
      }
    }
    ...
    const newTransport = new Farmer(transportData);
    ...
    res.status(201).json({
      ...
      data: {
        id: newTransport._id,
        name: newTransport.personalInfo.name,
        mobileNo: newTransport.personalInfo.mobileNo,
        role: newTransport.role
      }
    });

  } catch (error) {
    console.error('Error in registerTransport:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
