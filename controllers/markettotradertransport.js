...
exports.completeJourney = async (req, res) => {
  try {
    ...
    if (!orderId || !transporterId || !deliveryKey) {
      return res.status(400).json({
        success: false,
        message: "orderId, transporterId and deliveryKey are required",
      });
    }
    ...
    // ✅ Check transporter authorization
    if (
      order.marketToTraderTransport.transporterId?.toString() !==
      transporterId?.toString()
    ) {
    ...
    // ✅ Get stored delivery key (supports both locations)
    const storedKey =
      order.marketToTraderTransport.deliveryKey ||
    ...
    if (storedKey.toUpperCase() !== deliveryKey.toUpperCase()) {
      return res.status(400).json({
        success: false,
...
exports.generateTraderDeliveryKey = async (req, res) => {
  try {
    ...
    if (!order.marketToTraderTransport) {
      return res.status(400).json({
        success: false,
        message: "Transport not initiated",
      });
    }

    // Check if correct trader
    if (order.traderId?.toString() !== traderId?.toString()) {
      return res.status(403).json({
        success: false,
        message: "Unauthorized trader",
      });
    }

    // Generate key
    const generatedKey = "TRADER" + Math.floor(1000 + Math.random() * 9000);

    // ✅ VERY IMPORTANT — save inside correct nested object
    order.marketToTraderTransport.deliveryKey = generatedKey;
    order.marketToTraderTransport.updatedAt = new Date();
    ...
    res.status(200).json({
      success: true,
      message: "Delivery key generated successfully",
      deliveryKey: generatedKey,
    });

  } catch (error) {
    console.error("Error generating trader delivery key:", error);
    res.status(500).json({
      success: false,
      message: "Failed to generate delivery key",
      error: error.message,
